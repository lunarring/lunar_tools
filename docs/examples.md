...
| [`webrtc_sender.py`](../examples/webrtc_sender.py) / [`webrtc_receiver.py`](../examples/webrtc_receiver.py) | `comms` | Sender streams numpy frames, JPEG bytes, and JSON metadata over WebRTC and auto-hosts the signaling server; receiver attaches via the same host/port. |
