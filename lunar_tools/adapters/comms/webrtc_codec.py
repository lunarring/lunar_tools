...
class EncodedMessage:
    """Container describing a message prepared for WebRTC transport."""
...
def encode_message(address: str, payload: Any) -> EncodedMessage:
    """Serialise a payload for transport over a WebRTC data channel."""
...
def decode_message(message: bytes | str) -> Dict[str, Any]:
    """Decode a WebRTC data-channel message back into an envelope."""
    ...
    if kind == "ndarray":
        ...
        try:
            ...
            raise ValueError("Binary data does not match advertised ndarray shape") from exc
        payload = array
    else:
        payload = body
