...
class RestSignalingClient:
    """Simple polling REST client used to exchange SDP blobs."""
    ...
    def __init__(
        ...
        *,
        ...
        self._request_timeout = max(request_timeout, 1.0)
    ...
    def _wait_for_description(
        ...
        while True:
            ...
            if deadline is not None:
                ...
                if remaining <= 0:
                    return None
                wait = min(10.0, max(1.0, remaining))
...
class SimpleRestSignalingServer:
    ...
    def __init__(
        ...
        *,
        ...
        self._host = host
        self._port = port
        self._logger = logger if logger else create_logger(__name__ + ".signaling")
    ...
    def _build_handler(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def log_message(self, format: str, *args: Any) -> None:  # noqa: A003 - base name
                server._logger.debug("HTTP: " + format, *args)
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        self._logger.info("WebRTC signaling server listening on http://%s:%s", self._host, self._port)
    ...
    def _handle_post(self, handler: BaseHTTPRequestHandler) -> None:
        ...
        self._logger.info("Stored %s for session %s", role, session_id)
    ...
    def _handle_get(self, handler: BaseHTTPRequestHandler) -> None:
        ...
        if description is None:
            ...
            self._logger.debug("GET %s/%s waiting", session_id, role)
        else:
            self._logger.info("Served %s for session %s", role, session_id)
    ...
    def _send_response(handler: BaseHTTPRequestHandler, status: int, payload: Optional[Dict[str, Any]]) -> None:
        ...
        except BrokenPipeError:  # pragma: no cover - client dropped connection
            server = getattr(handler, "server", None)
            logger = getattr(server, "_logger", None)
            if logger:
                logger.debug("Client closed connection before response could be sent")


__all__ = ["RestSignalingClient", "SimpleRestSignalingServer"]
