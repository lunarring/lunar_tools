...
if TYPE_CHECKING:  # pragma: no cover - type hinting only
    ...
    import sounddevice as _sounddevice  # pragma: no cover
...
def _ensure_sounddevice():
    ...
    if _SOUNDDEVICE is None:
        try:  # pragma: no cover - optional dependency guard
            ...
            raise OptionalDependencyError("RealTimeVoice audio playback", ["audio"]) from exc
        _SOUNDDEVICE = sd_module
    return _SOUNDDEVICE


def _ensure_openai_realtime() -> None:
    global AsyncOpenAI, Session, AsyncRealtimeConnection, OPENAI_REALTIME_AVAILABLE
    if OPENAI_REALTIME_AVAILABLE:
        return
    try:  # pragma: no cover - optional dependency guard
        from openai import AsyncOpenAI as _AsyncOpenAI
        from openai.types.beta.realtime.session import Session as _Session
        from openai.resources.beta.realtime.realtime import AsyncRealtimeConnection as _AsyncRealtimeConnection
    except ImportError as exc:  # pragma: no cover
        raise OptionalDependencyError("OpenAI realtime", ["audio"]) from exc
    AsyncOpenAI = _AsyncOpenAI
    Session = _Session
    AsyncRealtimeConnection = _AsyncRealtimeConnection
...
class AudioPlayerAsync:
    def __init__(self, on_playback_complete: Optional[Callable[[], None]] = None, verbose: bool = False):
        ...
        self.stream: Optional["_sounddevice.OutputStream"] = None
        self.playing = False
        self._frame_count = 0
    ...
    def _ensure_stream(self) -> None:
        ...
        self.stream = sd_module.OutputStream(
...
class RealTimeVoice:
    def __init__(
        ...
        if not OPENAI_REALTIME_AVAILABLE or AsyncOpenAI is None:
            raise ImportError(
                "OpenAI realtime dependencies failed to initialise. "
                "Please verify your openai installation."
            )
        ...
        self._audio_stack_config = audio_stack_config
        self._audio_controller: Optional[AudioConversationController] = audio_controller
        self._audio_services = audio_controller.services if audio_controller else None
        ...
        self._audio_stack_boot_error: Optional[str] = None

    def _ensure_audio_controller(self) -> Optional[AudioConversationController]:
        if self._audio_controller:
            return self._audio_controller
        if self._audio_stack_config is None:
            return None
        try:
            services, synthesiser = bootstrap_audio_stack(self._audio_stack_config)
        except OptionalDependencyError as exc:  # pragma: no cover - optional dependency guard
            self._audio_stack_boot_error = str(exc)
            if self.verbose:
                print(f"Unable to bootstrap audio stack: {exc}")
            self._audio_stack_config = None
            return None
        self._audio_services = services
        self._audio_controller = AudioConversationController(services, synthesiser=synthesiser)
        return self._audio_controller

    @property
    def audio_controller(self) -> Optional[AudioConversationController]:
        return self._ensure_audio_controller()

    @property
    def audio_services(self) -> Optional["AudioServices"]:
        controller = self._ensure_audio_controller()
        if controller:
            return controller.services
        return self._audio_services
    ...
    def capture_transcript(
        ...
        *,
        ...
        if controller is None:
            raise RuntimeError(
                "AudioConversationController is not configured. "
                "Provide `audio_stack_config` or an `audio_controller` when initialising RealTimeVoice."
            )
        return controller.capture_transcript(
    ...
    async def _send_mic_audio(self, connection: AsyncRealtimeConnection) -> None:
        ...
        sd_module = _ensure_sounddevice()
        stream = sd_module.InputStream(channels=CHANNELS, samplerate=SAMPLE_RATE, dtype="int16")
        stream.start()
                                    ...
                                    if item.type == "message":
                                        for content in item.content:
                                            if content.type == "audio":
                                                ...
                                                had_audio = True
                                            elif content.type in {"output_text", "text"}:
                                                text_value = getattr(content, "text", None) or getattr(content, "value", None)
                                                if text_value:
                                                    text_chunks.append(text_value)
                                if not had_audio and text_chunks:
                                    text_response = " ".join(text_chunks).strip()
                                    if text_response:
                                        self.transcripts.append(
                                            TranscriptEntry(role="ai", message=text_response)
                                        )
                                        if self.on_ai_transcript:
                                            asyncio.create_task(self.on_ai_transcript(text_response))
                                    controller = self._ensure_audio_controller()
                                    if controller:
                                        try:
                                            controller.respond(
                                            ...
                                            if self.verbose:
                                                print(f"Audio controller playback failed: {exc}")
                                    elif self.verbose and self._audio_stack_boot_error:
                                        print(f"Audio fallback unavailable: {self._audio_stack_boot_error}")
                        else:
                            if self.verbose:
